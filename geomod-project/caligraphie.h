#ifndef CURVE_2D_LinearClosedCLOSED_H
#define CURVE_2D_LinearClosedCLOSED_H
...
class Curve2DLinearClosed : public Curve2D {
 ...
 void checkIfClosed(float frame){
     if(nbPts()>2){
 	 Vector2f ptS = evalAnimPt(get(0),frame);
     	 Vector2f ptE = evalAnimPt(get(nbPts()-1),frame);
	 if(ptS[0] != ptE[0] || ptS[1] != ptE[1])
	     add(ptS[0],ptS[1]);
     }
 }
};
...
#endif // CURVE_2D_LinearClosedCLOSED_H
